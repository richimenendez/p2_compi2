...
#       IDE MODIFICADO POR MÍ, REALIZADO POR:
#-----------------------------------------------------
...
from gramaticaC import ejecutar
import webbrowser
...
def exit_editor(event=None):
    if tkinter.messagebox.askokcancel("Salir", "Seguro que desea salir?"):
        root.destroy()
...
def open_file(event=None):
    ...
    if input_file_name:
        ...
        root.title('{} - {}'.format(os.path.basename(archivo), IDE))
        content_text.delete(1.0, END)
...
def write_to_file(archivo):
    try:
        content = content_text.get(1.0, 'end')
        with open(archivo, 'w') as the_file:
...
about_menu.add_command(label='Datos', command=compilar)
